...
class MeasurementThread(QThread):
    ...
    def __init__(self, resource_name, num_measurements, measurement_type, auto_zero, 
                 range_val="200mV", digits=8, mode="NPLC", gate_time=1.0, fourw_mode=0):
        super().__init__()
        ...
        self.digits = digits  # Used for NDIG and RESL command in NPLC mode
        self.mode = mode
        self.gate_time = gate_time  # seconds
        self.fourw_mode = fourw_mode  # 0=TRUE, 1=HIR for 4W Ohms
    ...
    def run(self):
        ...
        try:
            ...
            if self.mode == "NPLC":
                # Higher resolution (digits) requires more time
                timeout_ms = 60000 + (self.digits * 5000)  # More time for higher resolution
            else:
                timeout_ms = 60000 + int(self.gate_time * 1000)
            instrument.timeout = timeout_ms
            
            # Set termination characters for Fluke 8508A
            ...
            # 3. Set Measurement Function with Range (Fluke 8508A commands)
            # According to 8508A manual pages 4-13 to 4-15:
            # - Normal OHMS: OHMS command with TWO_WR or FOUR_WR option
            # - True OHMS: TRUE_OHMS command (always 4-wire, no TWO_WR/FOUR_WR needed)
            # - High Voltage OHMS: HIV_OHMS command
            
            # fourw_mode: 0=Normal 4W (OHMS FOUR_WR), 1=True OHMS (TRUE_OHMS), 2=HV OHMS (HIV_OHMS)
            func_map = {
                "DCV": "DCV",
                "ACV": "ACV", 
                "DCI": "DCI",
                "ACI": "ACI",
                "OHMS": "OHMS",           # 2-Wire Resistance (will add TWO_WR)
                "TOHMS": "OHMS",          # 4-Wire Resistance (will add FOUR_WR or use TRUE_OHMS)
            }
            ...
            # 4. Set Function and Range based on measurement type
            # For 4W OHMS (TOHMS): Use different commands based on fourw_mode
            if self.measurement_type == "TOHMS":
                if self.fourw_mode == 1:  # True OHMS mode
                    # TRUE_OHMS is always 4-wire, no need for FOUR_WR
                    if self.range_val == "AUTO":
                        print(f"DEBUG: Sending command: TRUE_OHMS")
                        instrument.write("TRUE_OHMS")
                        time.sleep(0.2)
                        print(f"DEBUG: Sending command: AUTO")
                        instrument.write("AUTO")
                    else:
                        cmd = f"TRUE_OHMS {self.range_val}"
                        print(f"DEBUG: Sending command: {cmd}")
                        instrument.write(cmd)
                elif self.fourw_mode == 2:  # High Voltage OHMS mode
                    if self.range_val == "AUTO":
                        print(f"DEBUG: Sending command: HIV_OHMS 1E6")
                        instrument.write("HIV_OHMS 1E6")
                        time.sleep(0.5)
                        print(f"DEBUG: Sending command: AUTO")
                        instrument.write("AUTO")
                    else:
                        cmd = f"HIV_OHMS {self.range_val}"
                        print(f"DEBUG: Sending command: {cmd}")
                        instrument.write(cmd)
                else:  # Normal 4-Wire OHMS mode (fourw_mode == 0) - uses TRUE_OHMS
                    if self.range_val == "AUTO":
                        print(f"DEBUG: Sending command: OHMS")
                        instrument.write("OHMS")
                        time.sleep(0.2)
                        print(f"DEBUG: Sending command: AUTO")
                        instrument.write("AUTO")
                    else:
                        cmd = f"OHMS {self.range_val}"
                        print(f"DEBUG: Sending command: {cmd}")
                        instrument.write(cmd)
                ...
                if self.range_val == "AUTO":
                    ...
                    instrument.write(cmd)
                time.sleep(0.3)
                print(f"DEBUG: Sending command: TWO_WR")
                instrument.write("TWO_WR")
            else:
                # Other measurement types (DCV, ACV, DCI, ACI)
                if self.range_val == "AUTO":
                    print(f"DEBUG: Setting AUTO range mode")
                    print(f"DEBUG: Sending command: {func_cmd}")
                    instrument.write(func_cmd)
                    time.sleep(0.2)
                    print(f"DEBUG: Sending command: AUTO")
                    instrument.write("AUTO")
                else:
                    cmd = f"{func_cmd} {self.range_val}"
                    print(f"DEBUG: Setting fixed range mode")
                    print(f"DEBUG: Sending command: {cmd}")
                    instrument.write(cmd)
            time.sleep(0.3)
            ...
            # 5. Set Auto-Zero
            if self.auto_zero:
                print(f"DEBUG: Sending command: AZERO ON")
                instrument.write("AZERO ON")
            else:
                print(f"DEBUG: Sending command: AZERO OFF")
                instrument.write("AZERO OFF")
            time.sleep(0.1)
            
            # 6. Set Resolution (NDIG: 5-8 for 5.5-8.5 digits)
            print(f"DEBUG: Sending command: NDIG {int(self.digits)}")
            instrument.write(f"NDIG {int(self.digits)}")
            time.sleep(0.1)
            
            # 6.1 Set FAST and FILT modes
            # FAST command: FAST_ON or FAST_OFF
            try:
                if self.fast:
                    print(f"DEBUG: Sending command: FAST_ON")
                    instrument.write("FAST_ON")
                else:
                    print(f"DEBUG: Sending command: FAST_OFF")
                    instrument.write("FAST_OFF")
            except Exception as e:
                print(f"DEBUG: Failed to set FAST mode: {e}")
            time.sleep(0.1)
            
            # FILT command: FILT_ON or FILT_OFF
            try:
                if self.filt:
                    print(f"DEBUG: Sending command: FILT_ON")
                    instrument.write("FILT_ON")
                else:
                    print(f"DEBUG: Sending command: FILT_OFF")
                    instrument.write("FILT_OFF")
            except Exception as e:
                print(f"DEBUG: Failed to set FILT mode: {e}")
            time.sleep(0.1)
            
            # 7. Configure triggering for Fluke 8508A
            # Set trigger source to bus (for software triggering)
            instrument.write("EXTRIG OFF")  # Disable external trigger
            instrument.write("TBUFF OFF")    # Disable trigger buffering for simple reads
            time.sleep(0.3)
            ...
            for i in range(self.num_measurements):
                ...
                try:
                    # For Integration mode, wait for gate_time before reading
                    if self.mode == "Integration":
                        time.sleep(self.gate_time)
                    
                    timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                    
                    # Fluke 8508A: Trigger a reading and query the result
                    # Method 1: Use "?" query which triggers and returns reading
                    # Method 2: Or use *TRG followed by reading
                    value_str = "0"
                    try:
                        # Try using "?" query - this is the standard Fluke 8508A way
                        print(f"DEBUG: Querying measurement with '?'")
                        value_str = instrument.query("?")
                        print(f"DEBUG: Received raw value: {value_str}")
                    except Exception as e:
                        print(f"DEBUG: '?' query failed: {e}")
                        # Fallback: try READ? or X? commands
                        try:
                            print(f"DEBUG: Querying measurement with 'READ?'")
                    ...
                    # Parse value - Fluke 8508A returns value with possible status info
                    value_str = value_str.strip()
                    # Remove any non-numeric prefix/suffix, keep scientific notation
                    try:
