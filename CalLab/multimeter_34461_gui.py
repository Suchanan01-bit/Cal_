"""
Keysight 34461A Multimeter GUI Application
A modern PyQt6-based GUI for controlling and monitoring Keysight 34461A 6.5-digit Multimeter
Based on HP 3458A template with layout from 34465A
"""
...
class MeasurementThread(QThread):
    ...
    def __init__(self, resource_name, num_measurements, measurement_type, gate_time, auto_zero, range_val="AUTO", mode="Integration", nplc=None, digits=6, sniffing_enabled=False, sniffing_interval=0):  # 34461A: max 6 digits
        super().__init__()
...
class PlotCanvas(FigureCanvas):
    ...
    def __init__(self, parent=None, width=8, height=4, dpi=100):
        ...
        self.gate_time = 1.0  # Default gate time (tau0)
    ...
    def set_graph_type(self, graph_type):
        """Set graph type: 'Trend', 'Histogram', or 'Allan Deviation'"""
        self.graph_type = graph_type
        self.plot_data()

    def set_gate_time(self, gate_time):
        """Set gate time (tau0) for Allan Deviation calculation"""
        self.gate_time = gate_time
        if self.graph_type == "Allan Deviation":
            self.plot_data()

    def calculate_adeviation(self, data, tau0):
        """Calculate Overlapping Allan Deviation"""
        import math
        n = len(data)
        if n < 9: # Minimum samples needed for meaningful ADEV
             return [], []
        
        # Max averaging factor m < (N-1)/2
        max_m = int((n - 1) / 2)
        m_values = []
        adev_values = []
        
        # Select m values (logarithmic spacing)
        # Powers of 2: 1, 2, 4, 8...
        m = 1
        while m <= max_m:
            m_values.append(m)
        ...
        # Add specific intermediate points if sparse
        if len(m_values) < 5 and max_m > 5:
        ...
        for m in m_values:
            ...
            count = 0
            # Overlapping sum
            # For frequency data y:
            # sum_{j=1}^{N-2m} (y_{j+m} - y_j)^2 / (2 * (N-2m) * m^2) ?? 
            # Wait, standard formula for frequency (y):
            # sigma_y^2(tau) = 1/(2(M-1)) * sum((y_i+1_bar - y_i_bar)^2)
            # where y_bar is average over tau. 
            # Let's use simpler non-overlapping for speed if N is large, or overlapping for better confidence.
            # Using simple Overlapping Allan Variance formula for measuring stability of y (frequency):
            # sigma_y^2(tau) = 1 / (2 * m^2 * (N - 2m)) * sum_{j=1}^{N-2m} (U_{j+2m} - 2U_{j+m} + U_j)^2 
            # where U is phase. We have frequency y.
            # Convert y to phase x? x(t) = integral(y(t) dt). x_k = sum_{i=1}^k y_i * tau0.
            # Then standard ADEV on phase data x is easier.
            
            # Let's use phase data approach:
            # 1. Integrate frequency to get phase x
            # x_k = tau0 * sum(y_i for i from 0 to k-1)
            # x_0 = 0
            
            # But calculating on Frequency directly (Overlapping):
            # sigma^2(m*tau0) = 1 / (2 * (N - 2m) * tau0^2) * sum_{j=1}^{N-2m} (x_{j+2m} - 2x_{j+m} + x_j)^2 
            # Re-expressed with y (average frequency over tau0):
            # x_{j+m} - x_j = tau0 * sum_{i=j}^{j+m-1} y_i
            # So term is (tau0 * sum_{i=j+m}^{j+2m-1} y_i - tau0 * sum_{i=j}^{j+m-1} y_i)^2
            # = tau0^2 * ( sum(y in bin2) - sum(y in bin1) )^2
            # Canceling tau0^2:
            # sigma^2 = 1 / (2 * (N - 2m) * m^2) * sum_{j=1}^{N-2m} ( sum_{k=0}^{m-1} (y_{j+m+k} - y_{j+k}) )^2
            # Wait, (sum(y_bin2) - sum(y_bin1))^2
            ...
            for j in range(N_minus_2m):
                # Sum measurement in bin 1: data[j] to data[j+m-1]
                # Sum measurement in bin 2: data[j+m] to data[j+2m-1]
                # Optimization: use slicing sum
                bin1 = sum(data[j : j+m])
                bin2 = sum(data[j+m : j+2*m])
                sum_diff_sq += (bin2 - bin1)**2
            
            sigma2 = sum_diff_sq / (2 * (n - 2*m) * (m**2))
            if sigma2 > 0:
                adev_values.append(math.sqrt(sigma2))
            else:
                adev_values.append(0)
                
        taus = [m * tau0 for m in m_values]
        return taus, adev_values

    def plot_data(self):
        """Update the plot with current measurements"""
        # Clear the axes but keep the grid/style if possible, or re-apply
        self.axes.clear()
        
        # Determine title and labels based on mode
        if self.mode == "FREQ":
            base_title = 'Frequency Stability'
            unit_label = 'Frequency (Hz)'
            color = '#9334e9'  # Purple for frequency
            avg_color = '#ea4335'
        else:
            base_title = 'Measurement'
            unit_label = f'Value ({self.unit})'
            color = '#1a73e8'  # Blue for others
            avg_color = '#ea4335'
            
        if not self.measurements:
            self.axes.text(0.5, 0.5, 'No data yet', ha='center', va='center',
                          fontsize=14, color='#9aa0a6', transform=self.axes.transAxes)
            self.axes.set_title(f"{base_title} ({self.graph_type})", fontsize=12, color='#3c4043', weight='bold', pad=15)
            self.axes.grid(True, alpha=0.3, linestyle='--', linewidth=0.5)
            self.fig.tight_layout()
            self.draw()
            return
        ...
        if self.graph_type == "Histogram":
            # Histogram Plot
            try:
                n, bins, patches = self.axes.hist(self.measurements, bins='auto', color=color, alpha=0.7, rwidth=0.9)
            except:
                self.axes.hist(self.measurements, bins=10, color=color, alpha=0.7, rwidth=0.9)
            
            # Add statistics and Gaussian Fit
            if len(self.measurements) > 1:
                ...
                self.axes.axvline(x=mean, color=avg_color, linestyle='--', linewidth=2, label=f'Mean: {mean:.8g}')
                
                # Plot Red Gaussian Bell Curve
                if stdev > 0:
                    try:
                        # Determine x range for the curve
                        x_min, x_max = self.axes.get_xlim()
                        
                        # Generate 100 points for smoothness
                        num_points = 100
                        step = (x_max - x_min) / (num_points - 1)
                        x_vals = [x_min + i * step for i in range(num_points)]
                        
                        # Calculate Gaussian PDF
                        # Scale factor = Area of histogram = Total Count * Bin Width
                        bin_width = bins[1] - bins[0]
                        scale_factor = len(data) * bin_width
                        
                        y_vals = []
                        prefactor = scale_factor / (stdev * math.sqrt(2 * math.pi))
                        
                        for x in x_vals:
                            exponent = -0.5 * ((x - mean) / stdev) ** 2
                            y = prefactor * math.exp(exponent)
                            y_vals.append(y)
                            
                        self.axes.plot(x_vals, y_vals, color='red', linewidth=2, label='Normal Dist.')
                    except Exception as e:
                        print(f"Error plotting Gaussian: {e}")
            elif len(self.measurements) > 0:
                 avg = sum(self.measurements) / len(self.measurements)
                 self.axes.axvline(x=avg, color=avg_color, linestyle='--', linewidth=2, label=f'Mean: {avg:.8g}')
            
            self.axes.set_xlabel(unit_label, fontsize=10, color='#3c4043', weight='bold')
            self.axes.set_ylabel('Count', fontsize=10, color='#3c4043', weight='bold')
            self.axes.legend(loc='upper right', fontsize=9, framealpha=0.9)
            
        elif self.graph_type == "Allan Deviation":
            # Allan Deviation Plot
            taus, adevs = self.calculate_adeviation(self.measurements, self.gate_time)
            
            if taus and adevs:
                self.axes.loglog(taus, adevs, 'o-', color='#d93025', linewidth=2, markersize=6)
                self.axes.grid(True, which="both", ls="-", alpha=0.3)
                self.axes.set_xlabel('Tau (s)', fontsize=10, color='#3c4043', weight='bold')
                self.axes.set_ylabel('Sigma (ADEV)', fontsize=10, color='#3c4043', weight='bold')
                
                # Turn off log scale manual override for ADEV as loglog handles it
            else:
                self.axes.text(0.5, 0.5, 'Need more samples (>9)', ha='center', va='center',
                              fontsize=14, color='#9aa0a6', transform=self.axes.transAxes)
                self.axes.set_xlabel('Tau (s)', fontsize=10, color='#3c4043', weight='bold')
                self.axes.set_ylabel('Sigma (ADEV)', fontsize=10, color='#3c4043', weight='bold')
            ...
            self.axes.plot(x, self.measurements, 'o-', color=color, 
                          linewidth=2, markersize=6, label='Measurements')
            
            if len(self.measurements) > 0:
                avg = sum(self.measurements) / len(self.measurements)
                self.axes.axhline(y=avg, color=avg_color, linestyle='--', 
                                linewidth=1.5, label=f'Average: {avg:.8g} {self.unit}')
            
            self.axes.set_xlabel('Measurement Number', fontsize=10, color='#3c4043', weight='bold')
            self.axes.set_ylabel(unit_label, fontsize=10, color='#3c4043', weight='bold')
            self.axes.legend(loc='upper right', fontsize=9, framealpha=0.9)
        ...
        self.axes.set_title(f"{base_title} - {self.graph_type}", fontsize=12, color='#3c4043', weight='bold', pad=15)
        # Grid is handled inside for loglog, but re-enable for others if needed override
        if self.graph_type != "Allan Deviation":
             self.axes.grid(True, alpha=0.3, linestyle='--', linewidth=0.5)
        
        # Add margins to ensure points aren't clipped at edges
...
class Keysight34461MultimeterGUI(QMainWindow):
    """Main GUI window for Keysight 34461A Multimeter application"""
    ...
    def __init__(self):
        ...
        # Range definitions for Keysight 34465A (Label, Unit, SCPI Value)
        self.range_map = {
            "DCV": [
                ("Auto", "V", "AUTO"),
                ("100 mV", "V", "0.1"),
    ...
    def init_ui(self):
        ...
        self.setWindowTitle("Keysight 34461A Multimeter Control Panel")
        ...
        # Title
        title = QLabel("📟 Keysight 34461A Multimeter Control Panel")
        ...
        if MATPLOTLIB_AVAILABLE:
            ...
            graph_type_label.setFont(QFont("Segoe UI", 10))
            graph_controls.addWidget(graph_type_label)
            
            self.graph_type_combo = QComboBox()
            self.graph_type_combo.setFont(QFont("Segoe UI", 10))
            self.graph_type_combo.setStyleSheet(self.get_input_style())
            self.graph_type_combo.addItems(["Trend Chart", "Histogram", "Allan Deviation"])
        ...
        self.status_bar.showMessage("✨ Ready - Keysight 34461A Multimeter Control")
    ...
    def create_connection_group(self):
        ...
        self.resource_combo.addItems([
            "USB0::0x2A8D::0x0523::MY12345678::0::INSTR",  # Keysight 34461A USB
            "TCPIP0::192.168.1.100::inst0::INSTR",         # LAN
            "GPIB0::22::INSTR"                             # GPIB
        ])
    ...
    def create_measurement_type_group(self):
        ...
        # Measurement types for Keysight 34461A (no TEMP/CAP)
        types = [
            ("⚡ DC Voltage", "DCV", "V"),
            ("〜 AC Voltage", "ACV", "V"),
            ("⚡ DC Current", "DCI", "A"),
    ...
    def create_settings_group(self):
        ...
        self.digit_combo.addItems(["4", "5", "6"])  # 34461A is 6.5 digit max
    ...
    def create_control_buttons(self):
        ...
        math_null_btn = QPushButton("Math Null")
    ...
    def on_type_changed(self, checked, type_name, unit):
        ...
        if checked:
            ...
            if MATPLOTLIB_AVAILABLE and hasattr(self, 'plot_canvas'):
                ...
                self.plot_canvas.set_mode(type_name)
                # Reset graph type? No, keep user desire.
                self.plot_canvas.plot_data()
    
    def on_graph_type_changed(self, text):
        """Handle graph type change"""
        if MATPLOTLIB_AVAILABLE and hasattr(self, 'plot_canvas'):
            type_map = {"Trend Chart": "Trend", "Histogram": "Histogram", "Allan Deviation": "Allan Deviation"}
            self.plot_canvas.set_graph_type(type_map.get(text, "Trend"))
    ...
    def execute_math_null(self):
        ...
        try:
            ...
            self.status_bar.showMessage("โ–๏ธ Executing Math Null...")
            self.results_text.append("\nโ–๏ธ Math Null Function")
            self.results_text.append("=" * 40)
            
            # Read current value as null reference
            current_val = inst.query("READ?")
            self.results_text.append(f"๐“ Current reading: {current_val.strip()}")
            ...
            self.results_text.append("โ… Math Null enabled")
            self.results_text.append("๐“ All subsequent readings will have this offset subtracted")
            
            inst.close()
            self.status_bar.showMessage("โ–๏ธ Math Null enabled successfully")
            
        except Exception as e:
            self.status_bar.showMessage(f"โ Math Null failed: {str(e)}")
    ...
    def execute_db_mode(self):
        ...
        try:
            ...
            self.status_bar.showMessage("[GRAPH] Enabling dB mode...")
            self.results_text.append("\n[GRAPH] dB Relative Measurement Mode")
            self.results_text.append("=" * 40)
            
            # Get current voltage as reference
            current_val = inst.query("READ?")
            ref_voltage = float(current_val.strip())
            self.results_text.append(f"[INFO] Reference voltage: {ref_voltage:.6f} V")
            ...
            self.results_text.append("[OK] dB mode enabled")
            self.results_text.append(f"[NOTE] dB = 20 * log10(V / {ref_voltage:.6f})")
            
            inst.close()
            self.status_bar.showMessage("[GRAPH] dB mode enabled successfully")
            
        except Exception as e:
            self.status_bar.showMessage(f"[ERR] dB mode failed: {str(e)}")
            self.results_text.append(f"\n[ERR] Error: {str(e)}")
    
    def execute_dbm_mode(self):
        """Execute dBm power measurement mode (reference: 600Ohm)"""
        if not PYVISA_AVAILABLE:
            QMessageBox.warning(self, "Error", "PyVISA is not installed!")
            return
        ...
        try:
            ...
            self.status_bar.showMessage("[MODE] Enabling dBm mode...")
            self.results_text.append("\n[MODE] dBm Power Measurement Mode")
            self.results_text.append("=" * 40)
            
            # Set reference impedance (600 Ohm is common for audio)
            ...
            self.results_text.append(f"[INFO] dBm reading: {reading.strip()} dBm")
            self.results_text.append("[OK] dBm mode enabled (Reference: 600 Ohm)")
            self.results_text.append("[NOTE] dBm = 10 * log10(V^2/R / 1mW)")
            
            inst.close()
            self.status_bar.showMessage("[MODE] dBm mode enabled successfully")
            
        except Exception as e:
            self.status_bar.showMessage(f"[ERR] dBm mode failed: {str(e)}")
            self.results_text.append(f"\n[ERR] Error: {str(e)}")
    ...
    def execute_minmax(self):
        ...
        try:
            ...
            self.status_bar.showMessage("[STATS] Reading Min/Max statistics...")
            self.results_text.append("\n[STATS] Min/Max Statistics")
            self.results_text.append("=" * 40)
            ...
            try:
                ...
                self.results_text.append(f"[MIN] Minimum: {min_val.strip()}")
            except:
                self.results_text.append("[MIN] Minimum: Not available")
            
            try:
                max_val = inst.query("CALC:AVER:MAX?")
                self.results_text.append(f"[MAX] Maximum: {max_val.strip()}")
            except:
                self.results_text.append("[MAX] Maximum: Not available")
            
            try:
                avg_val = inst.query("CALC:AVER:AVER?")
                self.results_text.append(f"[AVG] Average: {avg_val.strip()}")
            except:
                self.results_text.append("[AVG] Average: Not available")
            
            try:
                count = inst.query("CALC:AVER:COUN?")
                self.results_text.append(f"[COUNT] Count: {count.strip()}")
            except:
                self.results_text.append("[COUNT] Count: Not available")
            
            self.results_text.append("\n[OK] Min/Max statistics retrieved")
            
            inst.close()
            self.status_bar.showMessage("[STATS] Min/Max statistics read successfully")
            
        except Exception as e:
            self.status_bar.showMessage(f"[ERR] Min/Max failed: {str(e)}")
            self.results_text.append(f"\n[ERR] Error: {str(e)}")
    ...
    def check_dependencies(self):
        ...
        messages = []
        if not PYVISA_AVAILABLE:
            messages.append("[WARN] PyVISA not installed. Install with: pip install pyvisa pyvisa-py")
        if not MATPLOTLIB_AVAILABLE:
            messages.append("[WARN] Matplotlib not installed. Install with: pip install matplotlib")
        if messages:
            self.results_text.append("\n".join(messages))
            self.results_text.append("\n" + "="*60 + "\n")
    ...
    def refresh_resources(self):
        ...
        try:
            ...
            # Common defaults for 34465A
            defaults = [
                "USB0::0x2A8D::0x0523::MY12345678::0::INSTR",
            ...
            if resources:
                ...
                self.status_bar.showMessage("No resources found, using defaults")
                QMessageBox.warning(self, "No Resources", "No VISA resources found!\nAdded default examples.")
            
            # Add defaults if not present
            for default in defaults:
                if self.resource_combo.findText(default) == -1:
                    self.resource_combo.addItem(default)
    ...
    def test_connection(self):
        ...
        try:
            ...
            self.status_bar.showMessage(f"[OK] Connected: {idn[:50]}...")
            
        except Exception as e:
            QMessageBox.critical(self, "Connection Failed", f"Could not connect:\n{str(e)}")
            self.status_bar.showMessage(f"[ERR] Connection failed")
    ...
    def start_measurement(self):
        ...
        # Get selected measurement type (34461A has no TEMP/CAP)
        ...
        self.results_text.append("📟 Keysight 34461A Measurement Started")
        ...
        if measurement_type in ["DCV", "ACV"]:
            ...
            self.results_text.append("  * dBm = 10 * log10(V^2/R / 1mW)")
            self.results_text.append("  * dBm Reference Impedance: 600 Ohm")
        ...
        # Pass gate time to plot canvas for Allan Deviation
        if MATPLOTLIB_AVAILABLE and hasattr(self, 'plot_canvas'):
            # Approximation of gate time for ADEV calculation
            self.plot_canvas.set_gate_time(gate_time_sec)
        ...
        self.status_bar.showMessage(" Measuring in progress...")
    ...
    def on_measurement_complete(self, measurements):
        ...
        if measurements:
            ...
            if self.current_unit == "V" and avg != 0:
                ...
                if avg > 0:
                    ...
                    self.results_text.append(f"\n  * dBm Reference Impedance: {R_ref} Ohm")
        ...
        self.status_bar.showMessage(f"[OK] Measurement complete - {len(measurements)} readings")
    ...
    def on_error(self, error_msg):
        ...
        self.stop_btn.setEnabled(False)
        self.results_text.append(f"\n[ERR] Error: {error_msg}")
        self.status_bar.showMessage(f"[ERR] Error: {error_msg}")
    ...
    def clear_results(self):
        ...
        if MATPLOTLIB_AVAILABLE and hasattr(self, 'plot_canvas'):
            self.plot_canvas.clear_measurements()
        self.status_bar.showMessage("[INFO] Results cleared")
    ...
    def save_and_open_csv(self):
        ...
        try:
            ...
            filename = output_dir / "34461A_output.csv"
            ...
            try:
                ...
                self.status_bar.showMessage("— Closing open file...")
            ...
            self.status_bar.showMessage(f"[SAVED] Saved and opened: {filename}")
...
def main():
    ...
    window = Keysight34461MultimeterGUI()
