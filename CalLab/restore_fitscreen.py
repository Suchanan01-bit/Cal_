...
method_code = '''    def create_settings_group(self):
        ...
        # --- Row 0: Basic Settings (Range, Digits, AZERO, OCOMP) ---
        ...
        layout.addWidget(self.range_combo, 0, 1)

        # Digits (NDIG)
        digits_label = QLabel("NDIG:")
        digits_label.setFont(QFont("Segoe UI", 10))
        layout.addWidget(digits_label, 0, 2)
        ...
        layout.addWidget(self.digit_combo, 0, 3)
        ...
        layout.addWidget(self.auto_zero_check, 0, 4)
        ...
        layout.addWidget(self.offset_comp_check, 0, 5)

        # --- Row 1: Mode Selection (Combined with controls) ---
        ...
        layout.addWidget(self.gate_time_spin, 1, 2, 1, 2) # Span 2 columns
        ...
        layout.addWidget(self.nplc_spin, 1, 2, 1, 2)

        # Number of measurements
        num_label = QLabel("Readings:")
        num_label.setFont(QFont("Segoe UI", 10))
        layout.addWidget(num_label, 1, 4)
        ...
        layout.addWidget(self.acband_enable_check, 2, 0)
        ...
        layout.addWidget(self.acband_spin, 2, 1)
        ...
        layout.addWidget(self.lfilter_check, 2, 2)

        setacv_label = QLabel("SETACV:")
        setacv_label.setFont(QFont("Segoe UI", 10))
        layout.addWidget(setacv_label, 2, 3) 
        ...
        layout.addWidget(self.setacv_combo, 2, 4, 1, 2) # Span 2 columns
...
for line in lines:
    ...
    if not skip:
        # Also fix show() back to showMaximized()
        if "window.show()" in line and "window = HP3458MultimeterGUI()" not in line:
             new_lines.append(line.replace("window.show()", "window.showMaximized()"))
        else:
...
print("File restored to Fit Screen (Grid) layout successfully.")
